...
folderGraph = '/path/to/caligraph-instances_transitive-types.nt' # see http://caligraph.org/resources.html for file

files = os.listdir(folderGraph)

# for transitive types, rdf:type is important
...
for file in files:
    if '.nt' not in file:
        ...
        data.to_csv(folderGraph + '/transitive-types_' + str(counter_file) + '.csv', sep=";")
        counter_file += 1
