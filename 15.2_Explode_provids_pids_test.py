...
folder = '/path/to/9_FINAL/data/machine_learning/two_class/distance_test/Extract_provids/'
files = os.listdir(folder)


# loop through all files and explode entries
for file in files:
    # check if files is not already 'exploded'
    ...
    exploded_data['provid'] = exploded_data.provid.apply(lambda x: str(x).split('patterns:')[0])
        ...
        # export data
    exploded_data.to_csv('/path/to/9_FINAL/data/machine_learning/two_class/distance_test/exploded_pid_provids/'+file, sep=";")
